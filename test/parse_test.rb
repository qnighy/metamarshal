...
class ParseTest < Minitest::Test
  ...
  def test_parse_true
    assert_equal true, parse("\x04\x08T")
  end

  def test_parse_false
    assert_equal false, parse("\x04\x08F")
  end
  ...
  def test_parse_fixnum_long
    ...
    assert_equal 1234, parse("\x04\x08i\x02\xD2\x04")
    assert_equal 34567, parse("\x04\x08i\x02\x07\x87")
    assert_equal 12345678, parse("\x04\x08i\x03\x4E\x61\xBC")
    assert_equal 1073741823, parse("\x04\x08i\x04\xFF\xFF\xFF\x3F")
    assert_equal(-124, parse("\x04\x08i\xFF\x84"))
    assert_equal(-234, parse("\x04\x08i\xFF\x16"))
    assert_equal(-1234, parse("\x04\x08i\xFE\x2E\xFB"))
    assert_equal(-34567, parse("\x04\x08i\xFE\xF9\x78"))
    assert_equal(-12345678, parse("\x04\x08i\xFD\xB2\x9E\x43"))
    assert_equal(-1073741824, parse("\x04\x08i\xFC\x00\x00\x00\xC0"))
  end
  ...
  def test_parse_array
    assert_syn_isomorphic MetaArray.new([1, 2, 3]), parse("\x04\x08[\x08i\x06i\x07i\x08")
  end

  def test_parse_link
    cycle1 = MetaArray.new([]).tap { |a|
      a.data << a
    }
    cycle2 = MetaArray.new([]).tap { |a|
    ...
    non_shared = MetaArray.new([
      MetaArray.new([0]),
      MetaArray.new([0])
    ])
    shared = MetaArray.new([
      MetaArray.new([0])
    ] * 2)
